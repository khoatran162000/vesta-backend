...
async function generateStudentCode() {
    ...
    const lastStudent = await database_1.default.user.findFirst({
...
async function createUser(req, res) {
    try {
        ...
        if (role === "STUDENT") {
            ...
            const existingCode = await database_1.default.user.findUnique({ where: { studentCode: code } });
            if (existingCode)
                return api.error(res, `Mã học viên ${code} đã tồn tại`, 409);
            // Kiểm tra trùng email nếu có
            if (email) {
                const existingEmail = await database_1.default.user.findUnique({ where: { email } });
                if (existingEmail)
                    return api.error(res, "Email đã được sử dụng", 409);
            }
            const passwordHash = await bcryptjs_1.default.hash(password || "Student@123", 12);
            ...
            if (!email)
                return api.error(res, "Email bắt buộc cho tài khoản nhân sự");
            if (!password)
...
async function bulkCreateStudents(req, res) {
    try {
        ...
        const results = { created: 0, skipped: 0, errors: [], createdStudents: [] };
        for (const s of students) {
            ...
            if (s.email) {
                const existingEmail = await database_1.default.user.findUnique({ where: { email: s.email } });
                if (existingEmail) {
