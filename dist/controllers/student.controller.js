...
exports.getAttemptReview = getAttemptReview;
...
async function getDashboard(req, res) {
    try {
        ...
        const [totalAttempts, submittedAttempts, avgScore, recentAttempts, unreadNotifications] = await Promise.all([
            ...
            database_1.default.notification.count({ where: { userId: studentId, isRead: false } }),
        ]);
...
async function getExams(req, res) {
    try {
        ...
        const exams = await database_1.default.exam.findMany({
            where,
            orderBy: { createdAt: "desc" },
            include: {
        ...
        const examsWithAttempts = await Promise.all(exams.map(async (exam) => {
            const attemptCount = await database_1.default.examAttempt.count({
                where: { examId: exam.id, studentId },
            });
            const bestScore = await database_1.default.examAttempt.aggregate({
...
async function startExam(req, res) {
    try {
        ...
        if (exam.status !== "PUBLISHED")
            return api.error(res, "Đề thi chưa được xuất bản", 400);
        // Kiểm tra có bài đang làm dở không
        const existingAttempt = await database_1.default.examAttempt.findFirst({
        ...
        // Tạo lượt thi mới
        const attempt = await database_1.default.examAttempt.create({
...
async function saveAnswers(req, res) {
    try {
        ...
        const attempt = await database_1.default.examAttempt.findFirst({
            where: { id: attemptId, studentId, status: "IN_PROGRESS" },
        });
        if (!attempt)
            return api.error(res, "Lượt thi không tồn tại hoặc đã nộp", 404);
        await database_1.default.examAttempt.update({
            where: { id: attemptId },
            data: {
...
async function submitExam(req, res) {
    try {
        ...
        const updated = await database_1.default.examAttempt.update({
            where: { id: attemptId },
            data: {
                status: "SUBMITTED",
                endTime: new Date(),
...
async function getAttemptReview(req, res) {
    try {
        ...
        if (!attempt)
            return api.error(res, "Không tìm thấy bài làm", 404);
        const studentAnswers = (attempt.answers || {});
        const notes = (attempt.studentNotes || {});
