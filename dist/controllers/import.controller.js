...
const importStudentsFromCSV = async (req, res) => {
    try {
        ...
        if (lines.length < 2)
            return res.status(400).json({ success: false, message: "File CSV trống hoặc không hợp lệ" });
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (email) {
                ...
                if (dup) {
                    results.failed++;
                    results.errors.push({ row: i + 1, name: fullName, reason: `Email ${email} đã tồn tại` });
                    continue;
                }
            }
            ...
            if (dupPhone) {
                results.failed++;
                results.errors.push({ row: i + 1, name: fullName, reason: `SĐT ${phone} đã tồn tại` });
                continue;
            }
            ...
            let suffix = 1;
            while (exists) {
                studentCode = `${generateCode(fullName)}${suffix}`;
